...
class keyfigures:
    ...
    def run(self):
        ...
        plevels = pd.read_csv(self.paths[1], delimiter=';')
        keyfigures = pd.read_csv(self.paths[2], delimiter=';')
        attaskeyfigure = pd.read_csv(self.paths[3], delimiter=';')
        ...
        list1 = list(keyfigures['Input Key Figures']) #list of all inputs

        totalCommas = 0
        for i in list1: #checks for the string with the largest amount of commas
            try:
                currentCommas = i.count(',') #counts number of commas per string
                if currentCommas > totalCommas:
                    totalCommas = currentCommas
            except Exception as e:
                pass

        new = keyfigures['Input Key Figures'].str.split(',', n = totalCommas+1, expand = True) #splits the string on the hyphen
        
        for i in range(len(new.columns)): #adds all the inputs
            kfcalculationsfinal['Input' + str(i+1)] =  new[i]
            kfcalculationsfinal['Input' + str(i+1) + ' Type'] = np.nan #E
        
        

        df1 = pd.DataFrame(index=[0,1,2,3],columns=kfcalculationsfinal.columns) #adds 4 empty rows above 
        kfcalculationsfinal = df1.append(kfcalculationsfinal, ignore_index=True) #re-adds column names under 4 empty rows
        kfcalculationsfinal.iloc[3] = kfcalculationsfinal.columns
        ...
        worksheet.merge_range(0, 0, 3, len(column_names)-1, 'Key Figure Definition', workbook.add_format({'align': 'center', 'valign': 'vcenter', 'bold': True, 'color': 'red',
        'font_size': 20}))
        ...
        #add pictures and format
        worksheet1.merge_range(0, 0, 3, len(column_names1)-1, 'Key Figure Calculations', workbook.add_format({'align': 'center', 'valign': 'vcenter', 'bold': True, 'color': 'red'
    , 'font_size': 20}))
        worksheet1.insert_image(0,1,'/Users/sanjaymamidipaka/Downloads/csv_files/bizbrain.png')
        #worksheet1.insert_image(0,0,'/Users/sanjaymamidipaka/Downloads/csv_files/natureswaylogo.png')

        print(kfcalculationsfinal.head())
