...
class timeprofile:
    ...
    def run(self):
        # natures way
        timeprofile = pd.read_csv(self.paths[4], delimiter=';')
        ...
        merge_format = workbook.add_format({'align': 'left', 'bold': True, 'color': 'red', 'bg_color': 'white'})
        worksheet.merge_range(4, start_of_attributes, 4, len(column_names)-1, 'Attributes', merge_format)
        #change colors
        cell_format = workbook.add_format({'bold': True, 'color': 'red', 'bg_color': 'white', 'valign': 'vcenter', 'align': 'center', 'text_wrap': True})
        for i in range(len(column_names)):
            worksheet.write(4,i, column_names[i], cell_format)
        #add pictures and format
        worksheet.merge_range(0, 0, 3, len(column_names)-1, 'Time Profile', workbook.add_format({'align': 'center', 'valign': 'vcenter', 'bold': True, 'color': '', 'bg_color': 'white'
        , 'font_size': 20}))
