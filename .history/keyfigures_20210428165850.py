...
# Convert the dataframe to an XlsxWriter Excel object.
keyfiguresfinal.to_excel(writer, sheet_name='KF Calculations', index=False, startrow=1, header = False)

# Get the xlsxwriter objects from the dataframe writer object.
workbook = writer.book
worksheet = writer.sheets['KF General']
worksheet1 = writer.sheets['KF Calculations']
...
cell_format = workbook.add_format({'bold': True, 'color': 'white', 'bg_color': 'red', 'valign': 'vcenter', 'align': 'center', 'text_wrap': True})
for i in range(len(column_names)):
    worksheet1.write(4,i, column_names[i], cell_format)
#add pictures and format
worksheet1.merge_range(0, 0, 3, len(column_names)-1, 'Key Figure Definition', workbook.add_format({'align': 'center', 'valign': 'vcenter', 'bold': True, 'color': 'red', 'bg_color': 'white'
, 'font_size': 20}))
