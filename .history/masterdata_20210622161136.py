...
class masterdata:
    ...
    def run(self): #RENAME paattributes TO PA ATTRIBUTES
        ...
        #get columns for second file
        attributeidmasterdata = pd.DataFrame(masterdatafinal['Attribute ID'])
        #conduct vlookup facets
        merged_attr_id_info = pd.merge(attributeidmasterdata, extracolumnspaattributes, on=['Attribute ID'], how='left')
        merged_attr_id_info = merged_attr_id_info.drop('Attribute ID', axis=1) #dropping unnecessary column
        masterdatafinal = pd.concat([masterdatafinal, merged_attr_id_info], axis=1) #merge vlookup columns
        ...
        masterdatafinal.to_excel(self.writer, sheet_name='Master Data & Attributes', index=False, startrow=1, header = False)

        # Get the xlsxwriter objects from the dataframe writer object.
        workbook = self.writer.book
        worksheet = self.writer.sheets['Master Data & Attributes']
        ...
        worksheet.merge_range(0, 0, 3, len(column_names)-1, 'Master Data & Attributes', workbook.add_format({'align': 'center', 'valign': 'vcenter', 'bold': True, 'color': 'red',
        'font_size': 20}))
