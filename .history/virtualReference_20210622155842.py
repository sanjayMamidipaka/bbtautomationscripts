...
class virtualReference:
    ...
    def run(self):
        ...
        worksheet.merge_range(4, 0, 4, x_range_referenced_attributes, 'REFERENCED ATTRIBUTES', workbook.add_format({'align': 'center', 'valign': 'vcenter', 'bold': True, 'color': 'red'}))

        #merge 'join conditions' heading
        x_range_join_conditions = 10
        worksheet.merge_range(4, x_range_referenced_attributes+1, 4, x_range_join_conditions, 'JOIN CONDITIONS', workbook.add_format({'align': 'center', 'valign': 'vcenter', 'bold': True, 'color': 'red'}))

        #change format of 'comments' cell
        worksheet.write(4, x_range_join_conditions+1, '', workbook.add_format({'bold': True, 'color': 'red'}))
        worksheet.write(5, x_range_join_conditions+1, 'Comment', workbook.add_format({'bold': True, 'color': 'red'}))

        #change columns colors/formatting
        cell_format1 = workbook.add_format({'bold': True, 'color': 'red', 'valign': 'vcenter', 'align': 'center', 'text_wrap': True})
        for i in range(len(column_names)): #red and white
            worksheet.write(5,i, column_names[i], cell_format1)

        worksheet.merge_range(0, 0, 3, len(column_names)-1, 'Virtual & Reference Tables', workbook.add_format({'align': 'center', 'valign': 'vcenter', 'bold': True, 'color': 'red', 'bg_color': 'white'
        , 'font_size': 20}))
